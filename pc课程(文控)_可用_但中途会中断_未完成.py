...
mychrome.get("http://www.kaster.cn:7089/default/desks/users.admin/@zopen.desks:index")
mychrome.add_cookie({'name':'session_key','value':'e5fb254c5d2ef77a148f5b1eff67a408'})
mychrome.get("http://www.kaster.cn:7089/default/desks/users.admin/@zopen.desks:index")
...
def click_elements(li):
    ...
    if els:
        ...
        for el in els:
            search_terms = ['待审文件']
            if any(item in li.text for item in search_terms):#any() 函数：any() 函数接受一个可迭代对象作为参数，并返回该可迭代对象中的任何元素是否为真（非零、非空字符串等）。
                #item in li.text for item in search_terms：这是一个生成器表达式，用于遍历 search_terms 列表中的每个搜索项，检查它是否存在于 li.text 中。
                break
            try:
                el.click()
                                ...
                                #     break
                                        ...
                                        for tr in tbody.find_elements_by_tag_name("tr"):
                                            ...
                                            try:
                                                ...
                                                fields = ['name', 'code','ctime']
                                                values=(td,tr.get_attribute("data-uid"),current_datetime)
