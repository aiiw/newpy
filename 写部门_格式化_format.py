...
import xlrd
book=xlrd.open_workbook('excel/部门.xlsx') #打开Excel
...
tp={}
print(str(sheet.row_values(1)[0]))
for i in range(1,nrows):
	# print(sheet.row_values(i))#第一行内容
	# list1=["{:.0f}".format(sheet.row_values(j)[0]) for j in range(1,nrows)]
	list1=[sheet.row_values(j)[0] for j in range(1,nrows)]
print(list1)
for item in list1:
	sql='''update  t
set t.ExpiryDate='2023-01-01 00:00:00.000'
from T_HR_Department t
where t.ExpiryDate is  null and  t.DepartmentCode like'{}%'
'''.format(item)

# 	sql='''select * from T_HR_Department d where d.DepartmentCode like'{}%'
# '''.format(item)
	print(sql)
